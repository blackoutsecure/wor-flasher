...
function createLinuxArchive(root, version) {
  const archive = join(root, `WoR-Flasher-${version}-linux.tar.gz`);
  run("tar", ["-czf", archive, "-C", root, "wor-flasher"]);
  ...
  for (const script of [
    ...
    if (!listing.stdout.split("\n").includes(script)) {
      fail(`Linux archive is missing ${script}`);
    }
  }
}
...
function buildLinux(root) {
  ...
  copyRuntime(payload);
  createLinuxArchive(root, readProjectMetadata().product.version);
  writeReadme(root, [
    "WoR-Flasher Linux release artifact",
    "",
    "This directory is regenerated by npm run build.",
    "Publish WoR-Flasher-<version>-linux.tar.gz for Linux users.",
